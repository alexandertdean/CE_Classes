...
static int afsk_probe(struct platform_device *pdev)
{
	...
	struct afsk_data_t *afsk_dat=NULL;	// Data to be passed around the calls

	struct device_node *dn=NULL;		// Start of my device tree
	struct device_node *dn_enable=NULL;	// Enable pin properties
	struct device_node *dn_m_sb=NULL;	// Mark-Space bar pin properties
	struct device_node *dn_ptt=NULL;	// PTT properties
	struct device_node *dn_shdn=NULL;	// Shutdown properties
...
static int afsk_open(struct inode *inode, struct file *fildes)			
{
	if ((fildes->f_flags && O_RDONLY) || (fildes->f_flags && O_RDWR)) return -EINVAL;
	return 0;																			//Must be O_WRONLY, which is good
}



static long afsk_ioctl(struct file *fd, unsigned int request, unsigned long argp)			//changes buffer size of delimiter
{
	int errChk;
	u32 tempDelimCnt;
	
	errChk = mutex_lock_interruptible(&(afsk_data_fops-lock));
	if (errChk) {
			printk(KERN_INFO "Could not successfully get lock\n");
			return -EACCES;																//returns Permission denied
	}
	
	if (request == IOCTL_SET_DELIM) {
		if (afsk_data_fops->delim_buf != NULL) {													//checks if data is already allocated for delimiter
				kfree(afsk_data_fops->delim_buf);														//frees previously allocated delimiter
		}
		errChk = copy_from_user(&tempDelimCnt, (void __user *) argp, sizeof(argp));			//gets size of delimiter from user
		if (errChk) {
			printk(KERN_INFO "Error copying bytes from user");
			mutex_unlock(&(afsk_data_fops->lock));
			return -EFAULT;
		}
		afsk_data_fops->delim_cnt = tempDelimCnt;
		afsk_data_fops->delim_buf = kmalloc(tempDelimCnt, GFP_ATOMIC);							//allocate copy number of bytes and prevents sleeping
		memset(afsk_data_fops->delim_buf, AX25_DELIM, copy);							//sets copy number of bytes to AX25_DELIM (0x7E) in allocated memory space
	} else if (request == IOCTL_GET_DELIM) {
			tempDelimCnt = afsk_data_fops->delim_cnt;
			errChk = copy_to_user((void __user *) argp, &tempDelimCnt, sizeof(argp));			//outputs size of delimiter to user
			if (errChk) {
				printk(KERN_INFO "Error copying bytes to user");
				mutex_unlock(&(afsk_data_fops->lock));
				return -EFAULT;
			}
	} else {
...
int encode(char *data, int stuffOn, int len)
{
	...
	udelay(833); 
	printk(KERN_INFO "%c\n", state?'S':'M');
	return i + 1;					//returns number of bytes written
}


static ssize_t afsk_write(struct file *fildes, const char __user *buf, size_t nbytes, loff_t *offp)
{
	char *data;
	char state = 1;
	char oneCount = 0;													//used for counting 1's for bit stuffing
	char bit;															//counts number of bits to know when to increment stuffed array;
	int i;																//for loop iterator variable
	int j;																//iterator for stuffed array
	int errChk;
	
	data = (char *) kmalloc(nbytes, GFP_ATOMIC);				//allocates nbytes number of bytes without sleeping
	if (data == NULL) {
			printk(KERN_INFO "Error allocating memory for write");
			return -ENOMEM;
	}
	errChk = copy_from_user(data, buf, nbytes);
	if (errChk) {
		printk(KERN_INFO "Failure to get data");
		kfree(data);
		data = 0;
		return -EBADF;
	}
	
	errChk = mutex_lock_interruptible(&(afsk_data_fops->lock)); 						//lock resources from being used
	if (errchk) {
		printk(KERN_INFO "Could not successfully get lock");
		kfree(data);
		data = 0;
	...
	gpiod_set_value(afsk_data_fops->ptt, 1);							//enables ptt
	mdelay(5);
	gpiod_set_value(afsk_data_fops->enable, 1);							//enables enable pin, this along with ptt enabled means data can be transmitted
	
	errChk = encode(afsk_data_fops->delim_buf, afsk_data_fops->delim_cnt, AFSK_NOSTUFF);
	if (errChk != afsk_data_fops->delim_cnt) {
		printk(KERN_INFO "Error encoding first delimiter\n");
		kfree(data);
		data = 0;
		return -EIO;
	}
	errChk = encode(data, nbytes, AFSK_STUFF);
	if (errChk != afsk_data_fops->delim_cnt) {
		printk(KERN_INFO "Error encoding data\n");
		kfree(data);
		data = 0;
		return -EIO;
	}
	errChk = encode(afsk_data_fops->delim_buf, afsk_data_fops->delim_cnt, AFSK_NOSTUFF);
	if (errChk != afsk_data_fops->delim_cnt) {
		printk(KERN_INFO "Error encoding second delimiter\n");
		kfree(data);
		data = 0;
		return -EIO;
	}
	
	gpiod_set_value(afsk_data_fops->enable, 0); 						//Disable the enable, ending writing sequence
	mdelay(5); //delay 5ms
	gpiod_set_value(afsk_data_fops->ptt, 0); 							//disable ptt
	mutex_unlock(&(afsk_data_fops->lock));
	kfree(data);
	data = 0;
	return nbytes;
...
static int afsk_release(struct inode *inode, struct file *fildes)		//Code that is called upon close system call
{
	return 0;
}
